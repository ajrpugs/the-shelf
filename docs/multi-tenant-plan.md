...
- **5c — Google.** **Not blocked by SMTP** — OAuth sends no mail, so this is the one slice that needs nothing but dashboard configuration. The gate is already built for it (`AUTH_PROVIDERS` in `index.html`, Google entry present with `enabled: false`); shipping it is Google Cloud OAuth credentials pointed at `https://<project-ref>.supabase.co/auth/v1/callback`, enabling the provider in Supabase, and flipping the flag. **Settle same-email identity linking first** (Authentication → Settings): with it on, an existing Discord reader signing in with Google on the same address keeps their account; with it off they get a second account — empty shelf, no clubs, and their picks and reviews still attached to one they can't reach. That's 5d arriving early and it's the only hard part.

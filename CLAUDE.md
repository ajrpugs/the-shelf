...
- `clubs` — one seeded row (fixed id `8fdb4e0f-ea2f-4a45-9d9a-059a3292b3f8`, slug `the-guild`, name "The Guild", `visibility = 'public'`) plus a `club_id` column on `reads`/`shelf_state`/`shelf_reviews`/`shelf_comments`/`shelf_comment_reactions`, constant-defaulted to that same id. `shelf_users` stays deliberately unscoped — it's identity, one row per human regardless of how many clubs they're in. **Every query in the client and in all five edge functions is now filtered on a hardcoded `DEFAULT_CLUB_ID` constant** matching the seeded row (Phase 3.5 finished this; inserts name `club_id` explicitly rather than relying on the column default). It's a stand-in until Phase 4's per-club routing gives the client a real club to resolve — `parseRoute()` already captures a slug but nothing looks it up.

...
- `reads` — **exists but is not yet used by the running app.** Phase 0 groundwork for `docs/multi-tenant-plan.md`: a real table (backfilled from `shelf_state.data.history`) meant to eventually replace that jsonb array. Nothing reads or writes it yet — `history` is still the source of truth until a later cutover migration. See `supabase/functions/_shared/shelf-logic.mjs` for the draw/undo/rating/meeting logic that was extracted (behavior-preserving) out of `admin-update` in the same pass, now covered by `node --test supabase/functions/_shared/`.

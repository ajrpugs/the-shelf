...
**Data model (`supabase/schema.sql`)** — eight tables:
...
- `club_members` — **also inert.** Backing table for the `is_member()`/`is_librarian()` RLS policy functions a later slice will add; nothing reads or writes it yet. Backfilled once from `shelf_users` (book) + `shelf_librarians` (role) — **a one-time snapshot, not a live mirror**: nothing keeps it in sync with `shelf_users`/`shelf_librarians` going forward. Whichever slice starts writing real RLS policies against it must re-backfill fresh (or add real dual-writes) immediately beforehand, or it'll be gating access on stale membership data.

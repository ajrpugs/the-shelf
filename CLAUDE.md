...
- `club_members` — **the source of truth for who is in a club, their `role`, and their current `book`** (as of Phase 3.5; it was a best-effort mirror before). Backing table for the `is_member()` RLS policy function, for `admin-update`'s own librarian check (a plain `.eq("club_id", ...).eq("user_id", ...)` query against `role`, run with the service-role client — not an `is_librarian()` SQL function; nothing needs one of those yet since no RLS *policy* checks a role, only this one application-level query does), for the client's Admin-tab gate, and for `draw`'s eligible-reader pool. **The write direction is now the reverse of what it was**: the `club_members` write is the one that must succeed, and `shelf_users.book` / `shelf_librarians` are best-effort mirrors that nothing reads. A member's presence in this table (not their `role`/`book` values) is what `is_member()` checks.
  - The table has **no insert policy** (same reasoning as `shelf_librarians`' write side), so a new reader joins via **`join_default_club()`** — a security-definer RPC, execute granted to `authenticated` only, that can add nothing but *the caller* to *the seeded club* as a plain `member`. The client calls it from `onSignedIn` after `ensureUserRow`. Phase 4 replaces it with an invite-gated join; until then, "sign in and you're on the shelf" is what it preserves. **A reader with no row here is invisible to the app entirely** — not on the shelf, not on the wheel — so any new join path has to write it.
- `profiles` — identity separate from any one club (`id`, `display_name`, `avatar_url`, `discord_id`). Closes one of the two schema gaps `docs/multi-tenant-plan.md` Phase 1 was left owing. **Additive only, not yet consumed anywhere** — a one-time backfill snapshot from `shelf_users`, not a live mirror; `shelf_users` stays the live source of truth until something (Phase 5, more auth providers) actually reads from `profiles` instead. RLS mirrors `shelf_users`: read for all, insert/update only your own row.

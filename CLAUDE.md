...
- **`clubs.config`'s client and server normalizers are two separate copies, and they *will* drift if you forget one.** Same failure mode as `webhookFor()` above, different pair of files: `supabase/functions/_shared/club-config.mjs` is the source of truth, `index.html` carries its own `normalizeConfig`/`normalizeRatingProfile`. This has already happened once — `notify` landed server-side without the client copy, `clubConfig().notify` was `undefined`, and the Admin tab's notification card threw reading `.draw` off it. **Whenever you add a key to `normalizeConfig` in one file, add it in the other in the same change**, and check by hand (`normalizeConfig({})` in both should return the same shape) — nothing currently asserts this automatically.
